...
def Action( mask_,video_i, object_track, img2,frame_detected, D_size, ACTIONS) :

    global Lx_hand, Ly_hand, Lx_pan, Ly_pan, hey
    
    if object_track == 'pan' :  
        Lx=Lx_pan 
        Ly=Ly_pan
    if object_track == 'hand' : 
        Lx=Lx_hand 
        Ly=Ly_hand 
    contours, _ = cv2.findContours(mask_ ,cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    contour_sizes = [(cv2.contourArea(contour), contour) for contour in contours]
    cnts = max(contour_sizes, key=lambda x: x[0])[1]
    ...
    if M["m00"]>0:
        ...
        if frame_detected >= len(Lx):
            rotateX = deque(Lx)
            rotateY = deque(Ly)
            rotateX.rotate(-1)
            rotateY.rotate(-1)
            rotateX[D_size-1]=xS1
            rotateY[D_size-1]=yS1
            Lx=list(deque(rotateX))
            Ly=list(deque(rotateY))
        
        if frame_detected < len(Lx): 
            frame_detected_vec=frame_detected
            Lx[frame_detected_vec]=xS1
            Ly[frame_detected_vec]=yS1
        if object_track == 'pan' :  
            Lx_pan=Lx 
            Ly_pan=Ly
        if object_track == 'hand' : 
        ...
        if (Lx[D_size-1] != None) and ((frame_detected % 2) == 0) :
            Vector=[]
            Vector.append(Lx)
            Vector.append(Ly)
#            print(object_track,' : ',Vector)
            hey = DNNAnalysis_Lib.predict(object_track,D_size,ACTIONS,real_time=True,vector=Vector)
            class_found = int(hey[3])
            return class_found
...
video_folder='CNN_Dataset/MLP_validation_videos'
...
for video_i in list_videos[1:]:
    ...
    cfhp=0
    cfpp=0
    consecutive_det_hand=0
    consecutive_det_pan=0
    frame_detected_Pan=0
    frame_detected_Hand=0
...
#                    cv2.circle(img2, (couple[0], couple[1]), 7, (255, 255, 255), -1)
            ...
            if(class_detections[i]=='hand'):
                 frame_detected_Hand+=1
                 mask_det=(r['masks'][:,:,i]).astype('uint8')*255
#                 DNNAnalysis_Lib.centroid_action(mask_det,video_i, 'hand', img2,tracking_vector)
                 class_found_hand = Action( mask_det,video_i,  'hand', img2,frame_detected_Hand, N, ACTIONS_hand)                 
                 if ((frame_detected_Hand % 2) == 0):                
                     mask_det=(r['masks'][:,:,i]).astype('uint8')*255
    #                 DNNAnalysis_Lib.centroid_action(mask_det,video_i, 'hand', img2,tracking_vector)
                     class_found_hand = Action( mask_det,video_i,  'hand', img2,frame_detected_Hand, N, ACTIONS_hand)
                     
                     if class_found_hand == cfhp :
                         consecutive_det_hand+=1
                     if class_found_hand !=cfhp :
                         consecutive_det_hand=0
                     coordh=[class_found_hand,consecutive_det_hand]
                     print('hand',coordh)
                     with open("hand.csv","a", newline='') as trackingF:
                            tracking_info = csv.writer(trackingF, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                            tracking_info.writerow(coordh)
                    
                     if (consecutive_det_hand >= threshold):
                         if class_found_hand == 1 : paso_hand = 7 #cv2.putText(img2,'remover',(50,50),cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2) 
                         if class_found_hand == 2 : paso_hand = 10
                         if class_found_hand == 3 : paso_hand = 11
                     
                     cfhp=class_found_hand   
            
            
            
            if(class_detections[i]=='pan_steak'):
                 frame_detected_Pan+=1
                 mask_det=(r['masks'][:,:,i]).astype('uint8')*255
#                 DNNAnalysis_Lib.centroid_action(mask_det,video_i, 'pan', img2,tracking_vector)
                 class_found_pan=Action( mask_det,video_i,'pan', img2,frame_detected_Pan, N, ACTIONS_pan)
     
                 if ((frame_detected_Pan % 2) == 0):  
                    

                     if class_found_pan == cfpp :
                         consecutive_det_pan+=1
                     if class_found_pan != cfpp :
                         consecutive_det_pan=0
                    
                     coord=[class_found_pan,consecutive_det_pan]
                     print('pan',coord)
                     with open("pan.csv","a", newline='') as trackingF:
                            tracking_info = csv.writer(trackingF, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                            tracking_info.writerow(coord)
                    
                     if (consecutive_det_pan >= threshold):
                         if class_found_pan == 1 :  paso = 2  #cv2.putText(img2,'poner',(50,50),cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
                         if class_found_pan == 2 :  paso = 8  #cv2.putText(img2,'quitar',(50,50),cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
                         if class_found_pan == 3 :  paso = 6  #cv2.putText(img2,'saltear',(50,50),cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 2)
                         
                
                     cfpp=class_found_pan    
        ...
        Txt10='Skip'
        Txt11='Back'
        Txt_Steak='Steak in place'
        a=255;b=255;c=255;d=255;e=255;f=255;g=255;h=255;j=255;k=255;z=255;l=0;m=0;
        if(paso==2): a=0
        if(paso==3): b=0   
        if(paso==4): c=0
        if(paso==5): d=0
        ...
        if(paso==8): g=0 
        if(paso==9): h=0 
        if(paso_hand==10): j=0; l=255
        if(paso_hand==11): k=0; m=255
        if(steak==True): d=0
       
        
        cv2.putText(img2,Txt1,(10,25),cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
        ...
        cv2.putText(img2,Txt8,(10,235+offset),cv2.FONT_HERSHEY_COMPLEX, 1, (g, 255, g), 2) 
        
        cv2.putText(img2,Txt9,(10,270+offset),cv2.FONT_HERSHEY_COMPLEX, 1, (h, 255, h), 2)  
       
#        cv2.putText(img2,Txt10,(10,305+offset),cv2.FONT_HERSHEY_COMPLEX, 1, (j, 255, j), 2)  
#       
#        cv2.putText(img2,Txt11,(10,340+offset),cv2.FONT_HERSHEY_COMPLEX, 1, (k, 255, k), 2) 
        cv2.putText(img2,Txt10,(480,25),cv2.FONT_HERSHEY_COMPLEX, 1, (j,l,0), 2)  
#       
        cv2.putText(img2,Txt11,(600,25),cv2.FONT_HERSHEY_COMPLEX, 1, (k,m,0), 2)         
        cv2.rectangle(img2,(470,1),(690,35),(255,100,100),1)        
