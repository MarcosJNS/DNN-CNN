# -*- coding: utf-8 -*-
"""
Script del programa principal de creacion de imagenes sinteticas en el que al ejecutarlo puedes crear 
las imagenes pegando objetos sobre fondos. Las instrucciones del
funcionamiento estan explicadas dentro del main.
"""
...
def main():
    ...
    Al ejecutarlo en la ventana de openCV aparecera inicialmente el fondo seleccionado con "num_BG" y se le podra pegar objetos deseados.
    ...
    Sobre la ventana de openCV se puede realizar las siguientes operaciones con el raton:
        
        mover el raton: se ira visualizando el como quedaria el objeto en la imagen
        
        ruleta raton: se puede hacer mas pequeño/grande el objeto que se va a pegar
            
        boton_izquierdo: se pega el objeto en la imagen y se guarda su mascara en la instancia "resultado" de la clase "imagen_resultado"
                         que se ha creado. Con la informacion guardada en resultado se podria volver a reproducir las mascaras obtenidas
                         para asi no tener que guardarlas en memoria
                         
        boton_derecho:  vuelve un paso atras, eliminando el ultimo objeto pegado
        
    Las siguientes operaciones con el teclado:
        
        tecla 'esc':  se cierra la ventana y representa el resultado obtenido con  matplotlib, quedando el resultado almacenado en la variable
                      'resultado'
         
        tecla 'c': Se cambia la categoria de objeto que se va a pegar a la siguiente que aparece en la carpeta objects
        
        tecla 's': Se selecciona manualmente la categoria de objeto a la que se quiere cambiar, introduciendo su
                   ID por el terminal
    
        tecla 'espacio': se cambia la imagen del objeto que se desea pegar a otro de la misma clase
        
        tecla 'b': SE GUARDA LA IMAGEN creada hasta el momento y se pasa a otra con el siguiente fondo
...
#    gray = cv2.medianBlur(gray, 3) # 3x3 median filter
    ...
    _, contours, _ = cv2.findContours(dst ,cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
