...
import random
ROOT_DIR=os.path.join("c:\\","Users\marcos\Documents")
...
def centroid_action( mask_pan,video_i, object_track, img2,tracking_vector) :
    ...
    foldername = os.path.join(ROOT_DIR,"DNN_model\\data_"+object_track+"\\raw_data_"+object_track+"\\")    
    folderdrop = os.path.join(ROOT_DIR,"DNN_model\\data_"+object_track+"\\classified\\")  
...
def train_test_teasers_split(train_object,D_size,ACTIONS):
    ...
    foldername = os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\classified\\")
    folderdrop = os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_ready\\")    
    if not os.path.exists(os.path.dirname(foldername)):
      try:
        os.makedirs(os.path.dirname(foldername))
      except OSError as exc: # Guard against race condition
...
def just_split(train_object,D_size,ACTIONS)    :
    ...
    foldername = os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_test_split\\")
    folderdrop = os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_test_split\\splitted\\")   
    if not os.path.exists(os.path.dirname(foldername)):
      try:
        os.makedirs(os.path.dirname(foldername))
      except OSError as exc: # Guard against race condition
...
def pred_norm(pred_object,D_size,step):
    ...
    foldername = os.path.join(ROOT_DIR,"DNN_model\\data_"+pred_object+"\\data_prediction\\")    
    if not os.path.exists(os.path.dirname(foldername)):
      try:
        os.makedirs(os.path.dirname(foldername))
      except OSError as exc: # Guard against race condition
...
def train(train_object,D_size,ACTIONS,train_with_predict):
  ...
  foldername = os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\model_"+train_object+"\\")
  model_dir = os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\model_"+train_object+"\\model_info\\")
  if not os.path.exists(os.path.dirname(foldername)):
      try:
        os.makedirs(os.path.dirname(foldername))
      except OSError as exc: # Guard against race condition
  ...
  if train_with_predict==False:  
      TRAINING=os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_ready\\Action_train_"+ train_object +".csv")
      TEST=os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_ready\\Action_test_"+ train_object +".csv")
  if train_with_predict==True: 
      TRAINING=os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_ready\\Predict_train_"+ train_object +".csv")
      TEST=os.path.join(ROOT_DIR,"DNN_model\\data_"+train_object+"\\train_ready\\Predict_test_"+ train_object +".csv")    
   #oad datasets.
...
def predict(pred_object,D_size,ACTIONS):
    ...
    model_dir = os.path.join(ROOT_DIR,"DNN_model\\data_"+pred_object+"\\model_"+pred_object+"\\model_info\\")
    model_dir = os.path.join(ROOT_DIR,"DNN_model\\data_"+pred_object+"\\model_"+pred_object+"\\model_info\\")
    foldername = os.path.join(ROOT_DIR,"DNN_model\\data_"+pred_object+"\\data_prediction\\predict_"+pred_object+".csv")   
    TRAINING=os.path.join(ROOT_DIR,"DNN_model\\data_"+pred_object+"\\train_ready\\Action_train_"+ pred_object +".csv")
